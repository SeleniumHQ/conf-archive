...
class AmeBoxDimensions extends KoStandaloneControl {
    constructor(params, $element) {
        ...
        for (const name in SymmetricDimensionMap) {
            if (!SymmetricDimensionMap.hasOwnProperty(name)) {
                continue;
            }
    ...
    getInputObservable(key) {
        if (this.symmetricModeEnabled() && (key in this.symmetricValues)) {
    ...
    getSettingFor(key) {
        ...
        if (this.symmetricModeEnabled() && (key in SymmetricDimensionMap)) {
