...
(function (AmeStyleGenerator) {
    ...
    const builtinFunctions = {
        ...
        formatLength: function (args) {
            ...
            const numericValue = parseFloat(args.value);
            ...
            createCssStatements(configs) {
                ...
                for (const config of configs) {
                    if (isRuleSetConfig(config)) {
                        ...
                        results.push(new ConditionalAtRule(config.identifier, config.condition, this.createCssStatements(config.nestedStatements)));
                    }
                    else {
